...
const AppHeader: React.FC = () => {
  ...
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const baseRoute =
      user?.perfil === "EMPRESA" ? "/empresa/proyectos" : "/consultor/proyectos";
    router.push(query ? `${baseRoute}?q=${encodeURIComponent(query)}` : baseRoute);
  };
            ...
            <form onSubmit={handleSearchSubmit}>
              <div className="relative">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-12 pr-14 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-gray-400 focus:outline-hidden focus:ring-3 focus:ring-gray-900/5 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-white/30 dark:focus:ring-white/5 xl:w-[430px]"
                />
                <button
                  type="submit"
                  className="absolute right-2.5 top-1/2 inline-flex -translate-y-1/2 items-center gap-0.5 rounded-lg border border-gray-200 bg-gray-50 px-[7px] py-[4.5px] text-xs -tracking-[0.2px] text-gray-500 dark:border-white/[0.08] dark:bg-white/[0.05] dark:text-gray-400"
                >
                  <span>Ctrl</span>
                  <span>K</span>
                </button>
