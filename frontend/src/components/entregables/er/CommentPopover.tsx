...
import type { ComentarioER } from "@/lib/types/modelo-er.types";
...
interface CommentPopoverProps extends CommentPopoverState {
...
function CommentCard({ c, onResolve, onReopen }: { c: ComentarioER; onResolve: (id: string) => void; onReopen: (id: string) => void }) {
      ...
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
            <span className="text-[10px] font-semibold text-gray-700 dark:text-white/70">{c.autor_nombre}</span>
            <span className="text-[9px] text-gray-400 dark:text-white/25">{formatDate(c.created_at)}</span>
            {c.es_desactualizado && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full font-medium bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">Desactualizado</span>
            )}
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${c.autor_perfil === "CONSULTOR" ? "bg-[#28b8d5]/10 text-[#28b8d5]" : "bg-purple-100 text-purple-600 dark:bg-purple-500/10 dark:text-purple-400"}`}>
...
export default function CommentPopover({ x, y, nombre, comentarios, onClose, onAddComment, onResolve, onReopen }: CommentPopoverProps) {
